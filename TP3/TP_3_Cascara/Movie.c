    ...
    if(lista != NULL && len > 0)
    {
        ...
        if(cant <= len)
        {
            for(i = 0; i < cant; i++)
            {
                ...
                if(!buscarLugarLibre(lista, len, &indice) && this != NULL)
                {
                    *this = *(auxArray + i);
    ...
    if(lista != NULL && len > 0 && ruta != NULL)
    {
        ...
        if(pArch != NULL)
        {
            do
            {
                ...
                if(this != NULL)
                {
                    if(fread(this, sizeof(sMovie), 1, pArch) == 1)
                    {
                        *(lista + i) = this;
                        i++;
...
int mostrarListaPeliculas(sMovie** lista, int len)
{
    ...
    if(lista != NULL && len > 0)
    {
        ...
        for(i = 0; i < len; i++)
        {
            if(*(lista + i) != NULL && mostrarUnaPelicula(*(lista + i)))
            {
                break;
            }
        }
...
static int escribirListaPeliculas(sMovie** lista, int len, FILE* pArch)
{
    ...
    if(lista != NULL && len > 0 && pArch != NULL)
    {
        for(i = 0; i < len; i++)
        {
            if(*(lista + i) != NULL && escribirPelicula(*(lista + i), pArch))
            {
                break;
            }
        }
